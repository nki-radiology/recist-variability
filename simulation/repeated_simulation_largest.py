...
def simulation(n_patients = 100, 
            n_readers = 100, 
            reps = 100, 
            n_organs_range = range(1,11), 
            n_lesions_range = range(1,21), 
            mean_growth_range = range(-100,310,10), 
            var_pat_range = range(10,110,10), 
            var_org_range = range(10,110,10), 
            var_res_range = range(10,110,10),
            df_organs = 3, 
            df_lesions = 10, 
            df_mean_growth = 12.8, 
            df_var_pat = 44.55**2, 
    ...
    choice = pd.DataFrame()
    for k, var_array in enumerate(discord_choice):
        minidf = pd.DataFrame(var_array)
        choice = pd.concat([choice, minidf], axis=1)

    categ = pd.DataFrame()
    for k, var_array in enumerate(discord_categ):
        minidf = pd.DataFrame(var_array)
        categ = pd.concat([categ, minidf], axis=1)

    return choice, categ, categs, percents